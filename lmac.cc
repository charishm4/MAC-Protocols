...
static class MacLmacClass : public TclClass {
public:
       MacLmacClass() : TclClass("Mac/LMAC") {}
...
#ifdef JOURNAL_PAPER
void LmacUpdateNeighbTimer::expire(Event *e) {
        a_->handleUpdateNeighbTimer();
}
                                                                                                                                                            
void LmacAdaptiveListenTimer::expire(Event *e) {
        a_->handleAdaptiveListenTimer();
}
#endif

void LmacGeneTimer::expire(Event *) {
	a_->handleGeneTimer();
}

void LmacRecvTimer::expire(Event *) {
	stime_ = rtime_ = 0;
	a_->handleRecvTimer();
}

void LmacRecvTimer::sched(double time) {
	TimerHandler::sched(time);
	stime_ = Scheduler::instance().clock();
	rtime_ = time;
}

void LmacRecvTimer::resched(double time) {
	TimerHandler::resched(time);
	stime_ = Scheduler::instance().clock();
	rtime_ = time;
}

double LmacRecvTimer::timeToExpire() {
	return ((stime_ + rtime_) - Scheduler::instance().clock());
}

void LmacSendTimer::expire(Event *) {
	a_->handleSendTimer();
}

void LmacNavTimer::expire(Event *) {
	a_->handleNavTimer();
}

void LmacNeighNavTimer::sched(double time) {
	TimerHandler::sched(time);
	stime_ = Scheduler::instance().clock();
	rtime_ = time;
}

void LmacNeighNavTimer::expire(Event *) {
	stime_ = rtime_ = 0;
	a_->handleNeighNavTimer();
}

double LmacNeighNavTimer::timeToExpire() {
	return ((stime_ + rtime_) - Scheduler::instance().clock());
}

void LmacCsTimer::expire(Event *) {
	a_->handleCsTimer();
}

// if pending, cancel timer
void LmacCsTimer::checkToCancel() {
	if (status_ == TIMER_PENDING)
		cancel();
}

// void LmacChkSendTimer::expire(Event *e) {
//   a_->handleChkSendTimer();
// }

void LmacCounterTimer::sched(double time) {
	...
	// This is implemented to be in step with the counter used in actual lmac.
...
double LmacCounterTimer::timeToSleep() {
	return ((stime_ + tts_) - Scheduler::instance().clock()) ;
}

void LmacCounterTimer::expire(Event *) {
	tts_ = stime_ = 0;
...
#ifdef JOURNAL_PAPER
LMAC::LMAC() : Mac(), mhUpdateNeighb_(this),mhNav_(this), mhNeighNav_(this), mhSend_(this), mhRecv_(this), mhGene_(this), mhCS_(this), mhAdap_(this), syncFlag_(0) {
        int i;
#else
LMAC::LMAC() : Mac(), mhNav_(this), mhNeighNav_(this), mhSend_(this), mhRecv_(this), mhGene_(this), mhCS_(this), syncFlag_(0) {
#endif
	state_ = IDLE;
	...
	*/
	...
	//durSyncPkt_ = ((SIZEOF_LMAC_SYNCPKT) * 12 + 18) / 1.0e4 ;

	durSyncPkt_ =  (PRE_PKT_BYTES + (SIZEOF_LMAC_SYNCPKT * ENCODE_RATIO)) * byte_tx_time_ + 1;
	durSyncPkt_ = CLKTICK2SEC(durSyncPkt_);

	//durDataPkt_ = ((SIZEOF_LMAC_DATAPKT) * 12 + 18) / 1.0e4 ;
	durDataPkt_ = (PRE_PKT_BYTES + (SIZEOF_LMAC_DATAPKT * ENCODE_RATIO)) * byte_tx_time_ + 1;
	durDataPkt_ = CLKTICK2SEC(durDataPkt_);

	//durCtrlPkt_ = ((SIZEOF_LMAC_CTRLPKT) * 12 + 18) / 1.0e4;
	durCtrlPkt_ = (PRE_PKT_BYTES + (SIZEOF_LMAC_CTRLPKT * ENCODE_RATIO)) * byte_tx_time_ + 1;
	...
	// initialize neighbour table
        for (i = 0; i < LMAC_MAX_NUM_NEIGHBORS; i++) {
        ...
        for (i = 0; i < LMAC_MAX_NUM_SCHEDULES; i++) {
...
//               	dutyCycle_ = LMAC_DUTY_CYCLE;
	...
	if (!syncFlag_)
		...
		for (int i=0; i< LMAC_MAX_NUM_SCHEDULES; i++) {
			mhCounter_[i] = new LmacCounterTimer(this, i);
		...
		if ( selfConfigFlag_ == 1) {
                ...
                //printf("LMAC_UPDATE_NEIGHB_PERIOD: ............node %d %d at %.6f\n", index_, LMAC_UPDATE_NEIGHB_PERIOD, Scheduler::instance().clock());
                mhUpdateNeighb_.sched(LMAC_UPDATE_NEIGHB_PERIOD);
                //dump();
#else
		mhGene_.sched(t);
#endif
...
void LMAC::setMySched(Packet *pkt) 
{
	...
	if (pkt == 0) { // freely choose my schedule
		...
		struct lmac_sync_frame *pf = (struct lmac_sync_frame *)pkt->access(hdr_mac::offset_);
...
int LMAC::command(int argc, const char*const* argv)
{
	if (argc == 3) {
...
// XXXX lmac handler functions

void LMAC::handleSendTimer() {
	assert(pktTx_);
  
	struct hdr_lmac *sh = HDR_LMAC(pktTx_);
...
void LMAC::handleRecvTimer() {
	assert(pktRx_);
  
	struct hdr_cmn *ch = HDR_CMN(pktRx_);
	struct hdr_lmac *sh = HDR_LMAC(pktRx_);
	...
	case SYNC_PKT:
		...
		break;
	default:
		fprintf(stderr, "Unknown lmac pkt type, %d\n", sh->type);
		break;
	}
...
void LMAC::handleGeneTimer() 
{
	...
	if (state_ == WAIT_CTS) {  // CTS timeout
		...
		if (numExtend_ < LMAC_EXTEND_LIMIT) { // extend time
			printf("LMAC %d: no ACK received. Extend Tx time.\n", index_);
			numExtend_++;
...
#endif
		assert(dataPkt_);
		struct hdr_lmac *mh = HDR_LMAC(dataPkt_);
...
void LMAC::handleCounterTimer(int id) {
	...
	sched_1:
		...
		if (schedTab_[id].txData == 1 &&
			...
			struct hdr_lmac *mh = (struct hdr_lmac *)dataPkt_->access(hdr_mac::offset_);
			if ((u_int32_t)mh->dstAddr == MAC_BROADCAST)
				howToSend_ = BCASTDATA;
			else
				howToSend_ = UNICAST;
...
#ifdef JOURNAL_PAPER
void LMAC::handleUpdateNeighbTimer() {
        //printf("LMAC::handleUpdateNeighbTimer: ............node %d at %.6f\n", index_, Scheduler::instance().clock());
...
void LMAC::handleAdaptiveListenTimer() {
...
void LMAC::recv(Packet *p, Handler *h) {
	...
	if (state_ == CR_SENSE) {
		...
		// cancels only if timer is pending; lmac could be in CR_SENSE with timer cancelled
...
void LMAC::discard(Packet *p, const char* why)
{
	hdr_cmn *ch = HDR_CMN(p);
	hdr_lmac *sh = HDR_LMAC(p);
...
int LMAC::drop_RTS(Packet *p, const char* why) 
{
	struct lmac_control_frame *cf = (lmac_control_frame *)p->access(hdr_mac::offset_);
...
int LMAC::drop_CTS(Packet *p, const char* why) 
{
	struct lmac_control_frame *cf = (lmac_control_frame *)p->access(hdr_mac::offset_);
...
int LMAC::drop_DATA(Packet *p, const char* why) 
{
	hdr_lmac *sh = HDR_LMAC(p);
...
int LMAC::drop_SYNC(Packet *p, const char* why) 
{
	drop(p, why);
	return 1;
}
...
void LMAC::checkMySched()
{
        ...
        if (schedTab_[0].numNodes == 1 && numSched_ > 1 && numNeighb_ > 0) {
                for (i = 1; i < LMAC_MAX_NUM_SCHEDULES; i++) {
...
void LMAC::update_schedTab_neighbList()
{
...
void LMAC::update_neighbList()
{
        ...
        checkMySched();
        mhUpdateNeighb_.resched(LMAC_UPDATE_NEIGHB_PERIOD);
}

void LMAC::check_schedFlag()
{
        int i;
...
void LMAC::handleDATA(Packet *p) {

	// internal handler for DATA packet
	struct hdr_cmn *ch = HDR_CMN(p);
	struct hdr_lmac * sh = HDR_LMAC(p);
...
#ifdef JOURNAL_PAPER
void LMAC::handleSYNC(Packet *p) 
{

	struct lmac_sync_frame *sf = (struct lmac_sync_frame *)p->access(hdr_mac::offset_);
	...
	state_ = IDLE;
      	...
      	nodeId = LMAC_MAX_NUM_NEIGHBORS;
      	schedId = LMAC_MAX_NUM_SCHEDULES;
      	for (i = 0; i < LMAC_MAX_NUM_NEIGHBORS; i++) {
      	...
      	if (nodeId < LMAC_MAX_NUM_NEIGHBORS) {
         	if (neighbList_[nodeId].state == sf->state) {
				...
				//printf("#############################################################\n");
		   		...
		   		mhCounter_[0]->sched(sf->sleepTime);
                   		...
                   		for (j = 0; j < LMAC_MAX_NUM_NEIGHBORS; j++) {
      	...
      	if (schedId == LMAC_MAX_NUM_SCHEDULES) {  // unknow schedule
        	flag =1;
         	// add an entry to the schedule table
         	if (numSched_ < LMAC_MAX_NUM_SCHEDULES){
            		for (i = 0; i < LMAC_MAX_NUM_SCHEDULES; i++) {
      	...
      	if (nodeId == LMAC_MAX_NUM_NEIGHBORS) {  // a new node
         	// didn't find an empty entry in schedule table, just drop the new node
         	if (schedId == LMAC_MAX_NUM_SCHEDULES) return;
         	// add it to my neighbor list
         	if (numNeighb_ < LMAC_MAX_NUM_NEIGHBORS){
            	for (i = 0; i < LMAC_MAX_NUM_NEIGHBORS; i++) {
	...
	schedTab_[schedId].numNodes--;
        if (schedTab_[schedId].numNodes == 0)
        	...
        	if (schedId == LMAC_MAX_NUM_SCHEDULES) {
...
//printf("node: %d ..............data sent............\n",index_);
	...
	int i, j;
	int foundNeighb = 0;
	int schedId = LMAC_MAX_NUM_SCHEDULES;
	//double t = Scheduler::instance().clock();
	//printf("Recvd SYNC (not/f) at %d from %d.....at %.6f\n", index_, sf->srcAddr, t);
...
bool LMAC::txNextFrag(void* data)
{
...
bool LMAC::sendMsg(Packet *pkt, Handler *h) {
	struct hdr_lmac *mh = HDR_LMAC(pkt);
#ifdef JOURNAL_PAPER
	...
	if ((u_int32_t)mh->dstAddr == MAC_BROADCAST) {
		return (bcastMsg(pkt));
                ...
                int fragNum = ch->size_ / SIZEOF_LMAC_DATAPKT ;
...
bool LMAC::unicastMsg(int , Packet *p) {
...
//printf("node: %d ..............data sent............\n",index_);
	...
	struct lmac_control_frame *cf = (struct lmac_control_frame *)p->access(hdr_mac::offset_);
	struct hdr_cmn *ch = HDR_CMN(p);

	ch->uid() = 0;
	ch->ptype() = PT_LMAC;
	ch->size() = SIZEOF_LMAC_CTRLPKT;
...
bool LMAC::sendCTS(double duration) {
...
bool LMAC::sendDATA() {
	...
	struct hdr_lmac * sh = HDR_LMAC(dataPkt_);
...
bool LMAC::sendACK(double duration) {
	// construct ACK pkt
	Packet *p = Packet::alloc();
	struct lmac_control_frame *cf = (struct lmac_control_frame *)p->access(hdr_mac::offset_);
	struct hdr_cmn *ch = HDR_CMN(p);
...
bool LMAC::sendSYNC()
{
  ...
  Packet *p = Packet::alloc();
  struct lmac_sync_frame *cf = (struct lmac_sync_frame *)p->access(hdr_mac::offset_);
  struct hdr_cmn *ch = HDR_CMN(p);

  ch->uid() = 0;
  ch->ptype() = PT_LMAC;
  ch->size() = SIZEOF_LMAC_SYNCPKT;
  ch->iface() = UNKN_IFACE.value();
  ch->direction() = hdr_cmn::DOWN;
  ch->error() = 0;	/* pkt not corrupt to start with */
  
  cf->length = SIZEOF_LMAC_SYNCPKT;
  cf->type = SYNC_PKT;
...
void LMAC::sentDATA(Packet *p)
{
  struct hdr_lmac *mh = HDR_LMAC(p);
...
void LMAC::updateNeighNav(double d ) {
  double now = Scheduler::instance().clock();
  //double d = duration * 1.0e-6; // convert to sec
...
double LMAC::txtime(Packet *p)
{
  ...
  default:
    fprintf(stderr, "invalid lmac pkt type %d\n", sh->type);
    exit(1);
  }
